...
async function askCoach(system: string, prompt: string, language: 'ru' | 'en' = 'ru') {
  ...
  const { data, error } = await supabase.functions.invoke('ai', {
    body: {
      system: `${system} ${
        language === 'en'
          ? 'Keep JSON keys unchanged, but write all user-facing values in English.'
          : 'Все пользовательские значения пиши по-русски.'
      }`,
      prompt,
    },
  });
