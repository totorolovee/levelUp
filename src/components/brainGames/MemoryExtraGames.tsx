...
export function MissingItemGame({ isRussian, onComplete }: BrainGameProps) {
  ...
  const choose = (item: string) => {
    const next = correct + Number(item === current.missing);
  ...
  return <section className="brain-game">
    ...
    {!show && <div className="game-choice-row">{choices.map((item) => <button key={item} onClick={() => choose(item)} type="button">{item}</button>)}</div>}
  </section>;
}
...
export function ReverseSequenceGame({ isRussian, onComplete }: BrainGameProps) {
  ...
  const check = () => {
    const next = correct + Number(answer === [...sequence].reverse().join(''));
  ...
  return <section className="brain-game">
    ...
    {!show && <form onSubmit={(event) => { event.preventDefault(); check(); }}>
      <input autoFocus inputMode="numeric" maxLength={sequence.length} onChange={(event) => setAnswer(event.target.value.replace(/\D/g, ''))} value={answer} />
      <button disabled={answer.length !== sequence.length} type="submit">{isRussian ? 'Готово' : 'Done'}</button>
    </form>}
  </section>;
}
...
export function GrowingMatrixGame({ isRussian, onComplete }: BrainGameProps) {
  ...
  return <section className="brain-game">
    ...
    <div className="pattern-grid">{Array.from({ length: 25 }, (_, index) =>
      <button className={(show && targets.has(index)) || chosen.has(index) ? 'active' : ''} key={index} onClick={() => choose(index)} type="button" />)}</div>
  </section>;
}
